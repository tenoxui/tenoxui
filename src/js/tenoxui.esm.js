...
class makeTenoxUI {
  ...
  pseudoStyles(type, value, unit, pseudoEvent, revertEvent) {
    // store the initial styles for selected element
    const initialStyle = this.element.style.getPropertyValue(this.camelToKebab(this.styles[type]));
    // applyStyle helper
    const applyStyle = () => {
      this.addStyle(type, value, unit);
    };
    ...
    const revertStyle = () => {
      this.element.style.setProperty(this.camelToKebab(this.styles[type]), initialStyle);
    };
...
function use(customConfig) {
  ...
  if (customConfig.property) {
    allProps = Object.assign(Object.assign({}, allProps), Object.assign({}, ...customConfig.property));
  }
}
