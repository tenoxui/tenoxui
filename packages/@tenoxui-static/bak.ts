...
// Updates to the TenoxUI class to handle conditional property types

// First, extend your types to support the new property format
...
export class TenoxUI {
  ...
  // Fix for the toKebabCase method to handle non-string inputs
  private toKebabCase(str: any): string {
  ...
  // Also, let's modify the addStyle method to ensure cssRules is properly handled
  public addStyle(
    className: string,
    ...
    if (Array.isArray(cssRules)) {
      const combinedRule = cssRules
        .map((prop: string) => {
          ...
          return value ? `${this.toKebabCase(propStr)}: ${value}` : this.toKebabCase(propStr)
        })
        .join('; ')
      styleSet.add(combinedRule)
    } else {
      // Ensure cssRules is a string
      const rulesStr = typeof cssRules === 'string' ? cssRules : String(cssRules)
      styleSet.add(value ? `${rulesStr}: ${value}` : rulesStr)
    }

    return this
  }

  // And let's update the processShorthand method to ensure proper handling of conditional properties
  public processShorthand(
    ...
    // Handle special case for [prop1,prop2] syntax
    ...
    // New code to handle arrays of conditional properties
    if (
      properties &&
      ...
      'type' in properties[0]
    ) {
      // Find the matching condition based on value type
      const matchingCondition = properties.find((condition) => {
        if (condition.type === '<color>' && this.isColorValue(value)) {
          return true
        }
        if (condition.type === '<number>' && this.isNumericValue(value)) {
          return true
        }
        ...
        // Add more type conditions as needed
        return false
      })

      if (matchingCondition) {
        const { property, value: valueTemplate } = matchingCondition

        const processedValue = valueTemplate
          ? valueTemplate.replace(/\{0}/g, finalValue).replace(/\{1}/g, finalSecValue || '')
          : finalValue

        return {
          className: `${type}-${value}${unit}`,
          cssRules: Array.isArray(property) ? property : this.toKebabCase(String(property)),
    ...
    // Existing code for handling other property formats
    if (properties) {
      ...
      return {
        className: `${type}-${value}${unit}`,
        cssRules: Array.isArray(properties) ? properties : this.toKebabCase(String(properties)),
        value: finalValue,
        prefix
      }
    }
  ...
  private isColorValue(value: string | undefined): boolean {
    ...
    if (value.startsWith('#') && /^#([0-9A-F]{3}){1,2}$/i.test(value)) return true

    // Check for rgb/rgba
    if (value.startsWith('rgb')) return true

    // Check if there's a color in this.values
    if (this.values && typeof this.values === 'object' && this.values[value] !== undefined) {
      const colorValue = this.values[value]
      if (
  ...
  private isNumericValue(value: string | undefined): boolean {
    if (!value) return false

    // Check if it's a pure number
    if (/^-?\d+(\.\d+)?$/.test(value)) return true

    // Add more checks if needed, like em, rem, px values
    if (/^-?\d+(\.\d+)?(px|em|rem|%|vh|vw)$/.test(value)) return true
