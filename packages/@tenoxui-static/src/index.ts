...
export class TenoxUI {
	...
	private getTypePrefixes(): string {
		...
		const classNames = this.getAllClassNames(classRegistry)
		return [...propertyTypes, ...classNames].sort((a, b) => b.length - a.length).join('|')
	}

	// generate regex pattern for class name parsing
	private generateClassNameRegEx2(): RegExp {
		return new RegExp(
			`(?:([a-zA-Z0-9-]+|\\[[^\\]]+\\]|\\([^)]+\\)|\\{[^}]+\\}):)?(${this.getTypePrefixes()}|\\[[^\\]]+\\])(?:-(-?(?:\\d+(?:\\.\\d+)?)|(?:[a-zA-Z0-9_]+(?:-[a-zA-Z0-9_]+)*(?:-[a-zA-Z0-9_]+)*)|(?:#[0-9a-fA-F]+)|(?:\\[[^\\]]+\\])|(?:\\$[^\\s]+))([a-zA-Z%]*)(?:\\/(-?(?:\\d+(?:\\.\\d+)?)|(?:[a-zA-Z0-9_]+(?:-[a-zA-Z0-9_]+)*(?:-[a-zA-Z0-9_]+)*)|(?:#[0-9a-fA-F]+)|(?:\\[[^\\]]+\\])|(?:\\$[^\\s]+))([a-zA-Z%]*))?)?`
		)
	}

	// parse any class names and divide them into parts
	public parseClassName2(
		className: string
	):
		| [
	...
	private processApplyObject(styles: Record<string, StyleValue>, parentSelector: string = '') {
		Object.entries(styles).forEach(([selector, value]) => {
			if (typeof value === 'string') {
				...
				if (selector.startsWith('@') && !ignoreQueries.some(query => selector.startsWith(query))) {
