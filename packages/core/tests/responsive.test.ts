...
describe("Responsive feature/apply styles on different screen", () => {
  ...
  it("should matching the breakpoints from each screenSize", () => {
    ...
    expect(styler.matchBreakpoint(bp, bp.name, 400)).toBe(true); // still around small screen
    expect(styler.matchBreakpoint(bp, bp.name, 650)).toBe(false); // outside small screen

    // around medium size
    bp = { name: "md", min: 640, max: 768 };
    expect(styler.matchBreakpoint(bp, bp.name, 639)).toBe(false); // behind medium screen size
    expect(styler.matchBreakpoint(bp, bp.name, 657)).toBe(true); // around medium size

    // breakpoints start from 640px to infinite
    bp = { name: "md", min: 640 };
    expect(styler.matchBreakpoint(bp, bp.name, 400)).toBe(false);
    expect(styler.matchBreakpoint(bp, bp.name, 650)).toBe(true);
    expect(styler.matchBreakpoint(bp, bp.name, 9909)).toBe(true);

    // breakpoint from 0 to 768px
    bp = { name: "md", max: 768 };
    expect(styler.matchBreakpoint(bp, bp.name, 400)).toBe(true);
    expect(styler.matchBreakpoint(bp, bp.name, 640)).toBe(true);
    expect(styler.matchBreakpoint(bp, bp.name, 769)).toBe(false);
